...
MAX_CHARS = 8000
DENY_TOKENS = ("\\windows", "\\program files", "ntuser.dat", ".ssh",
               "appdata\\roaming\\microsoft", ".env", "login data")
...
def main() -> None:
    ...
    if any(tok in low for tok in DENY_TOKENS):
        print(json.dumps({"error": "percorso non consentito"})); return
    p = Path(raw)
    if not p.is_file():
        print(json.dumps({"error": "file inesistente"})); return
    if p.stat().st_size > 5_000_000:
        print(json.dumps({"error": "file troppo grande (>5MB)"})); return
    try:
        text = p.read_text(encoding="utf-8", errors="replace")[:MAX_CHARS]
    except Exception as exc:
        print(json.dumps({"error": str(exc)})); return
    print(json.dumps({"ok": True, "name": p.name, "content": text,
