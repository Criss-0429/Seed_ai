...
class OnboardingEngine:
    ...
    def __init__(self, memory, llm=None, *, provider_required: bool = False,
                 provider_ready=None):
        self.memory = memory
        self.llm = llm
    ...
    def opening_prompt(self) -> str:
        ...
        if phase == "provider":
            return (
                "Prima delle domande personali configura e valida un provider BYOK. "
                "Ollama Cloud e consigliato e offre un piano Free; OpenRouter e "
                "Vercel AI Gateway sono alternative PAYG. Apri Impostazioni Provider, "
                "inserisci la tua key e completa il test reale."
            )
    ...
    def handle(self, text: str, *, episode_id: int | None = None) -> OnboardingReply:
        ...
        if phase == "provider":
            if self.provider_ready():
                state["phase"] = "story"
                self._save(state)
                self.memory.add_event("onboarding_provider_ready", {})
                return OnboardingReply(self.opening_prompt())
            return OnboardingReply(self.opening_prompt())
        if phase == "story":
    ...
    def _handle_consent(self, norm: str, state: dict[str, Any]) -> OnboardingReply:
        if norm in _ACCEPT:
            ...
            state["phase"] = (
                "provider"
                if self.provider_required and not self.provider_ready()
                else "story"
            )
            self._save(state)
