...
class Telemetry:
    ...
    def build_report(self) -> dict:
        ...
        voice = self._voice_summary(events)
        salience = self._salience_summary()
        daemon = self._daemon_summary(events)
        worker = self._worker_summary(events)
        observation = self._observation_summary(events)
    ...
    def _voice_summary(self, events: list[dict]) -> dict:
        """S11: aggregati uso/costo/errori voce. Mai audio, mai transcript."""
        stt = [e["payload"] for e in events if e["kind"] == "voice_stt"]
        tts = [e["payload"] for e in events if e["kind"] == "voice_tts"]
        return {
            "stt_calls": len(stt),
            "stt_ok": sum(1 for s in stt if s.get("ok")),
            "tts_calls": len(tts),
            "tts_ok": sum(1 for t in tts if t.get("ok")),
            "tts_fallbacks": sum(1 for t in tts if t.get("fallback") and t.get("ok")),
            "tts_chars": sum(int(t.get("chars", 0)) for t in tts if t.get("ok")),
            "consent_changes": sum(1 for e in events if e["kind"] == "voice_consent"),
        }
    ...
    def _knowledge_summary(self) -> dict:
        ...
        return {
            ...
            "by_type": by_type,
            "predictions": self._predictions_summary(),
    ...
    def _predictions_summary(self) -> dict:
        """K4: aggregati predizioni + Brier sulle risolte. Mai testo."""
        preds = self._memory.all_predictions()
        resolved = [p for p in preds if p["outcome"] in ("confirmed", "refuted")]
        brier = None
        if resolved:
            brier = round(sum(
                (float(p["probability"]) - (1.0 if p["outcome"] == "confirmed" else 0.0)) ** 2
                for p in resolved) / len(resolved), 4)
        return {
            "total": len(preds),
            "open": sum(1 for p in preds if p["outcome"] == "open"),
    ...
    def _daemon_summary(self, events: list[dict]) -> dict:
        """D1: aggregati daemon. Solo conteggi e flag dei confini, mai topic_ref
        o testo. Le decisioni vengono dai battiti (gia' aggregati)."""
        heartbeats = [e["payload"] for e in events if e["kind"] == "daemon_heartbeat"]
        decisions = {"emitted": 0, "suppressed": 0, "silenced": 0, "expired": 0}
        for hb in heartbeats:
            for key, value in (hb.get("decisions") or {}).items():
                decisions[key] = decisions.get(key, 0) + int(value)
        last = heartbeats[-1] if heartbeats else {}
        queue_status: dict[str, int] = {}
        for row in self._memory.proactivity_queue_items():
            queue_status[row["status"]] = queue_status.get(row["status"], 0) + 1
        return {
            "starts": sum(1 for e in events if e["kind"] == "daemon_started"),
            "stops": sum(1 for e in events if e["kind"] == "daemon_stopped"),
            "heartbeats": len(heartbeats),
            "last_tick": last.get("tick"),
            "last_queue_depth": last.get("queue_depth"),
            "decisions": decisions,
            "queue_status": queue_status,
            "write_actions": 0,            # D1: zero azioni agentiche di scrittura
            "supervised_process_only": True,
            "os_service": False,
        }

    def _worker_summary(self, events: list[dict]) -> dict:
        """D2: aggregati worker READ-only. Solo conteggi/flag: action, esito,
        dry_run, blocked. Mai output o argomenti."""
        invoked = [e["payload"] for e in events if e["kind"] == "worker_invoked"]
        blocked = [e["payload"] for e in events if e["kind"] == "worker_blocked"]
        by_action: dict[str, int] = {}
        for inv in invoked:
            action = inv.get("action", "?")
            by_action[action] = by_action.get(action, 0) + 1
        # D4: write-safe aggregati (mai contenuto).
        writes = [e["payload"] for e in events if e["kind"] == "worker_write_invoked"]
        write_blocked = [e["payload"] for e in events if e["kind"] == "worker_write_blocked"]
        return {
            "invocations": len(invoked),
            "ok": sum(1 for inv in invoked if inv.get("ok")),
            "dry_runs": sum(1 for inv in invoked if inv.get("dry_run")),
            "blocked": len(blocked),
            "denied": sum(1 for b in blocked if b.get("denied")),
            "by_action": by_action,
            "read_write_actions": 0,       # D2 read lane: zero scrittura
            "write_safe": {
                "invocations": len(writes),
                "ok": sum(1 for w in writes if w.get("ok")),
                "rolled_back": sum(1 for w in writes if w.get("rolled_back")),
                "blocked": len(write_blocked),
                "denied": sum(1 for b in write_blocked if b.get("denied")),
            },
        }

    def _observation_summary(self, events: list[dict]) -> dict:
        """D-OBS: aggregati observation lane. Classe/categoria/azione e conteggi,
        mai redacted_ref o contenuto grezzo."""
        processed = [e["payload"] for e in events if e["kind"] == "observation_processed"]
        by_action: dict[str, int] = {}
        by_class: dict[str, int] = {}
        for p in processed:
            by_action[p.get("action", "?")] = by_action.get(p.get("action", "?"), 0) + 1
            by_class[p.get("obs_class", "?")] = by_class.get(p.get("obs_class", "?"), 0) + 1
        return {
            "processed": len(processed),
            "candidates": by_action.get("candidate", 0),
            "remembered_silently": by_action.get("remember_silently", 0),
            "discarded": by_action.get("discard", 0),
            "by_class": by_class,
            "consent_changes": sum(1 for e in events if e["kind"] == "observation_consent"),
            "revocations": sum(1 for e in events if e["kind"] == "observation_revoked"),
            "write_actions": 0,            # READ-only
            "read_only": True,
        }
