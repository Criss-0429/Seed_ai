...
## Connettori, MCP e capability generate

La disponibilita di un MCP, plugin, package o adapter non costituisce prova di
affidabilita. Prima dell'attivazione sono richiesti:

- provenienza e publisher verificati quando possibile;
- versione, digest e dependency lock fissati;
- snapshot di schema e descrizione dei tool;
- scansione statica, dipendenze, licenze, PII e segreti;
- rete limitata a destinazioni dichiarate;
- test sintetici e avversariali;
- verifica contro token passthrough, SSRF, prompt injection e scope eccessivi;
- quarantena automatica su drift di schema, digest, dipendenze o autorita.

MCP e adapter locali vengono eseguiti in processo o container ristretto. Le
credenziali restano nel connector host separato: nessun tool generato riceve
password o token. L'installazione di dipendenze avviene in bundle o ambiente
isolato e riproducibile, mai direttamente nell'ambiente applicativo principale.
