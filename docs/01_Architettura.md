...
[User Knowledge Ontology](../seed-knowledge-graph/corpus/jarvis/wiki/Jarvis_User_Knowledge_Ontology.md).
...
### Selective Capability Forge

Il programma futuro `19_Selective_Capability_Forge_Plan.md` estende l'Evolution
Lab con confini separati:

- `Need & Fitness Engine`: decide se vale la pena imparare qualcosa e confronta
  sempre anche `non fare nulla`;
- `Connector Discovery & Vetting`: confronta capability esistenti, skill, MCP,
  API, plugin, file exchange, CLI e adapter custom;
- `Capability Builder`: costruisce artefatti isolati senza credenziali;
- `Independent Evaluator`: produce evidenza, non promozione;
- `Connection Broker`: conserva credenziali e crea connessioni comprensibili
  all'utente;
- `Capability Activation Authority`: puo attivare automaticamente soltanto
  entro autorita gia concesse.

Il connector host e separato dai tool generati. Possiede le credenziali e
espone soltanto operazioni tipizzate entro l'authority envelope; builder,
descendant e MCP generati non ricevono token o password.
...
[Agent Harness Best Practices](../seed-knowledge-graph/corpus/jarvis/wiki/Agent_Harness_Best_Practices.md)
...
## Flusso di una capability emergente

```text
workflow evidence locale
-> need hypothesis
-> fitness decision e alternative
-> connector discovery/vetting
-> capability plan
-> build isolato
...
-> outcome, drift, rollback e lineage
```

Se l'autorita richiesta non e rappresentabile o non e un sottoinsieme
deterministico dell'autorita gia concessa, il flusso fallisce chiuso e richiede
