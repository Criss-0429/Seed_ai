...
def run_tool(capability_dir: Path, input_payload: dict, timeout: int = 30) -> RunResult:
    ...
    try:
        proc = subprocess.run(
            _python_cmd(tool_path),
            input=json.dumps(input_payload, ensure_ascii=False),
            capture_output=True, text=True, encoding="utf-8",
            cwd=str(workspace), env=_minimal_env(), timeout=timeout,
            creationflags=subprocess.CREATE_NO_WINDOW if os.name == "nt" else 0,
        )
    except subprocess.TimeoutExpired:
        return RunResult(False, stderr=f"timeout {timeout}s", timed_out=True)
    except Exception as exc:
        return RunResult(False, stderr=str(exc))

    if proc.returncode != 0:
        return RunResult(False, stderr=proc.stderr[-2000:])
    try:
        return RunResult(True, output=json.loads(proc.stdout or "{}"),
                         stderr=proc.stderr[-2000:])
    except json.JSONDecodeError:
        return RunResult(False, stderr=f"output non-JSON: {proc.stdout[:200]}")
