...
> brand reale restano owner. Design autorevole in `FrameworkUtenti/SEED_UI`.

## Sorgenti di design (autorevoli)

`FrameworkUtenti/SEED_UI/`:
...
- colore guadagnato/hue onboarding sono supportati dal manifest ma richiedono
  eventi reali promossi per maturare: non vengono simulati;
- observation context-aware resta OFF e permission-gated; D3 isolamento esterno,
  D5 delega realmente isolata e D6 invio gateway reale restano limiti del piano
  daemon, non vengono mascherati dalla UI.
...
`FrameworkUtenti/SEED_UI/*`, `09_Personalita_Compatibile.md`,
